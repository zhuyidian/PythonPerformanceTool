...
class MeminfoManager:
    ...
    def write_to_excel(self, excel_path_name, txt_path_name, analyse_resource_image: AnalyseResourceImage,target_keywords,target_path,target_path_name):
        ...
        analyse_resource_image.do_meminfo_image('total_ram.jpg', 'total_ram', self.collection_time_list, list(map(float, total_ram_highs)), min,
                                        max, average,target_keywords,target_path)
        ...
        for bean in self.process_meminfo_list:
            ...
            if target_keywords in process_name:
                file_target.write(f'{bean.pid_set} ' + f'{process_name}(MB): ' + (','.join(process_highs_str)) + '\n')
                file_target.write(
                    f'{bean.pid_set} ' + f'{process_name}(MB): ' + f'min({min}) ' + f'max({max}) ' + f'average({average})' + '\n')
            # 生成image
            x_list = []
            for i in range(len(bean.time_list)):
                value = bean.front_back_list[i] + ' ' + bean.time_list[i]
                x_list.append(value)
            analyse_resource_image.do_meminfo_image(f'{process_name}.jpg', process_name, x_list, list(map(float, process_highs)),min,max,average,target_keywords,target_path)
